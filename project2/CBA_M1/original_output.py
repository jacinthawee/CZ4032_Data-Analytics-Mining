...
def m1_without_prune(data_path, scheme_path, minsup=0.01, minconf=0.5):
    ...
    cars = rule_generator(training_dataset, minsup, minconf,False)
    end_time = time.time()
    cba_rg_runtime = end_time - start_time

    start_time = time.time()
    classifier_m1 = M1(cars, training_dataset, False)
    end_time = time.time()
    ...
    print(f"Accuracy without pruning: {acc}")
    print(f"Car's run time without pruning: {cba_rg_runtime}")
    print(f"M1's run time without pruning:{cba_cb_runtime} ")
...
if __name__ == "__main__":
    ...
    for name in name_list:
        ...
        m1_without_prune(test_data_path, test_scheme_path)
        print()